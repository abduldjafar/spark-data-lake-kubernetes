...
builder = (
    pyspark.sql.SparkSession.builder.appName(apps_name)
...
# ET Process
videos_selected_field = datas.select(
    "title", "category_id", "views", "likes", "dislikes"
)
id_selected_field = datas_json_for_data_lake.select(
    "id", "snippet.*"
).withColumnRenamed("title", "category")

get_percentage = (
    videos_selected_field.join(
        id_selected_field,
        videos_selected_field.category_id == id_selected_field.id,
        "inner",
    )
    .select("title", "category", "views", "likes", "dislikes")
    .withColumn("likes", (col("likes") / col("views") * lit(100)))
    .withColumnRenamed("likes", "likes_percentage")
    .withColumn("likes_percentage", round(col("likes_percentage"), 2))
    .withColumn("dislikes", (col("dislikes") / col("views") * lit(100)))
    .withColumnRenamed("dislikes", "dislikes_percentage")
    .withColumn("dislikes_percentage", round(col("dislikes_percentage"), 2))
    .withColumn(
        "not_voted_percentage",
        lit(100) - (col("likes_percentage") + col("dislikes_percentage")),
    )
...
most_dislikes_film = get_percentage.distinct().sort(col("dislikes_percentage").desc())

most_dislikes_film.show()

most_likes_film = get_percentage.distinct().sort(
    col("likes_percentage").desc(), col("views").desc()
)

most_likes_film.show()

# write data into GCS
most_not_voted_film.limit(5).write.mode("overwrite").format("delta").save(
    gcs_data_destination + "/five_most_not_voted_film"
)
most_dislikes_film.limit(5).write.mode("overwrite").format("delta").save(
    gcs_data_destination + "/five_most_dislikes_film"
)
most_likes_film.limit(5).write.mode("overwrite").format("delta").save(
    gcs_data_destination + "/five_most_likes_film"
)